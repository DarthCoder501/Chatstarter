...
export const create = authenticatedMutation({
  ...
  handler: async (ctx, { username }) => {
    ...
    const directMessageMembersForCurrentUser = await ctx.db
      .query("directMessageMembers")
      .withIndex("by_user", (q) => q.eq("user", ctx.user._id))
      .collect();
    const directMessageMembersForOtherUser = await ctx.db
      .query("directMessageMembers")
      .withIndex("by_user", (q) => q.eq("user", ctx.user._id))
      .collect();
    const directMessage = directMessageMembersForCurrentUser.find((dm) =>
      directMessageMembersForOtherUser.find(
